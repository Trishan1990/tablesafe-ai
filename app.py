...
# -----------------------------
    ...
    pre_alert = fp_df[fp_df["Week"] < alert_dt]
    false_positives = pre_alert[pre_alert["Above Threshold"]]
    ...
    with f2_col:
        ...
        <div class="card">
            ...
            <div class="metric-caption">above threshold, no recall</div>
        </div>
        """, unsafe_allow_html=True)
    with f3_col:
    ...
    fig_fp = px.line(fp_df, x="Week", y="Composite Risk", markers=True,
                     title="Risk Timeline — False Positive Annotation")
    ...
    if len(false_positives) > 0:
        ...
        st.warning(f"{len(false_positives)} week(s) exceeded the threshold before confirmed alert. These require human review in live deployment.")
    else:
        st.success("✅ Zero false positives detected. The system only exceeded the 65% threshold during the confirmed pre-recall escalation window.")

    st.write("")
    st.markdown("""
    <div class="card">
        <h3 style="color:#2dd4bf;">Interpretation</h3>
        <p>A false positive = composite risk ≥ 65% but no recall within the following 8 weeks.
        Low false positive rates validate that signal fusion is precise and reduces unnecessary alerts.</p>
        <p><b>Industry benchmark:</b> Single-signal systems typically produce 15–30% false positive rates.
        TableSafe AI multi-signal fusion targets below 10%.</p>
    </div>
    """, unsafe_allow_html=True)
